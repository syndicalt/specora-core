...
def generate_config(ir: DomainIR) -> GeneratedFile:
    ...
    lines = [
        ...
        "",
        "# Connection pool. The ceiling is this container's concurrency limit for",
        "# database work, so it has to be tunable per deployment rather than baked in.",
        ...
        "    raise RuntimeError(",
        '        f"DATABASE_POOL_MIN_SIZE ({DATABASE_POOL_MIN_SIZE}) exceeds "',
        '        f"DATABASE_POOL_MAX_SIZE ({DATABASE_POOL_MAX_SIZE}); the pool cannot "',
        '        "be created."',
        "    )",
        "",
        "# Postgres cancels a statement that runs past this and the connection goes",
        "# back to the pool. Without it a single runaway query holds a connection",
        "# until it finishes, and enough of them starve the pool.",
